...
%===========================================================
\subsection{STP}
Chcemywybrać interfejs, który zostanie przełączony w stan "Alternate". W tym celu zwiększamy koszt połączenia z jednego z interfejsów.
\begin{lstlisting}
S3>enable
S3#conf t
S3(config)#interface Ethernet 4/1
S3(config-if)#spanning-tree cost 200
S3(config-if)#exit
S3(config)#exit
\end{lstlisting}
